...
- [x] Cierre de caja: archiva pedidos (asigna `id_reporte`, no borra), liquida domiciliarios, calcula consumo real de insumos — `Caja::generar()`. **Corregido en Fase 6:** el frontend YA mostraba un dialogo de "confirmar cierre" (`requestConfirmarCierre`/`ConfirmDialog`), pero era cosmetico — el archivado real ya habia ocurrido antes, al pedir la "vista previa" (que en realidad llamaba a `POST /cash-closing` directo). Se agrega `Caja::previsualizar()` + `GET /api/cash-closing/preview` (solo lectura, mismo calculo, sin ningun INSERT/UPDATE) para el primer paso, y `confirmarCierre()` en el frontend ahora es la UNICA llamada que persiste (`POST /cash-closing`, sin cambios en ese endpoint). Cancelar el dialogo o cerrar el modal de vista previa ya no deja nada archivado a medias. Verificado en vivo: preview no crea fila en REPORTE_CAJA ni toca `id_reporte` de ningun pedido (confirmado con 2 llamadas identicas a preview + conteo en BD); confirmar si crea el reporte y archiva solo los pedidos entregados del turno.
- [x] Soft delete de personal (`activo=0`, nunca `DELETE`) — `Personal::eliminarSuave()`, usado por `StaffController::destroy()`. Verificado: ya estaba correcto, sin cambios.
